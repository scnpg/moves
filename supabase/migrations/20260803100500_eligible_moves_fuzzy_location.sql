-- Adds fuzzy_lat/fuzzy_lng: a ~1.1km-precision pin that's always visible
-- (once a move has any location at all), so the map can show an
-- approximate marker for moves the caller hasn't joined yet. The existing
-- exact lat/lng stay gated behind host/approved-member status - "exact
-- address hidden until you join" is still enforced, just at a finer grain
-- now (approximate area vs. exact point) rather than all-or-nothing.
