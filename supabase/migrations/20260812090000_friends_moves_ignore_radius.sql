-- Friends' Moves should never be hidden by the discovery radius - only a
-- stranger's Open Move (no social-graph connection to the caller at all)
-- needs to be nearby to show up. Was: supabase/migrations/20260809110000_move_excluded_users.sql -
-- only the final AND clause (the ST_DWithin radius check) changes, adding
-- three bypasses (self-hosted, close friend, any friend/mutual degree)
-- ahead of it. Client-side sort (src/app/(tabs)/index.tsx) now sorts by
-- distance_m when available for both tabs, so a friend's far-away Move
-- still sorts to the bottom of the list rather than the top - "shown
-- regardless of distance, nearest first" - it just isn't excluded outright.
...
begin
  ...
  where m.status = 'active'
    ...
    and (
      ...
      or m.host_id = p_user_id
      or public.is_close_friend_of(m.host_id, p_user_id)
      or public.friendship_degree(p_user_id, m.host_id) is not null
